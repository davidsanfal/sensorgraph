...
from sensors.utils.graph_utils import strip_dataset
...
class GraphUtilsTestCase(TestCase):
    ...
    def test_strip_dataset_day(self):
        result = strip_dataset(Sensor.objects.all(),
                               'thermometer',
    ...
    def test_strip_dataset_week(self):
        result = strip_dataset(Sensor.objects.all(),
                               'thermometer',
                               datetime.strptime('30121999',"%d%m%Y").date(),
                               datetime.strptime('03012000',"%d%m%Y").date(),
    ...
    def test_strip_dataset_month(self):
        result = strip_dataset(Sensor.objects.all(),
                               'thermometer',
                               datetime.strptime('30121999',"%d%m%Y").date(),
                               datetime.strptime('03012000',"%d%m%Y").date(),
